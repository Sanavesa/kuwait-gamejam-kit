...
---
I want to undo my recent changes and go back to a saved checkpoint. Do it the **safe, non-destructive** way (never delete my saved history). **Keep git invisible to me:** talk in save points and checkpoints, never commits or git, and don't show me raw git commands or output.
...
- **Always confirm with me before rolling back; never undo without my explicit go-ahead.** This is a drastic change, so name the exact checkpoint we'd land on, warn me in ONE short line that it discards any changes I've made **since my last save**, and **wait for me to say yes** before touching any files. Even if I named a checkpoint in "$ARGUMENTS", confirm first.
