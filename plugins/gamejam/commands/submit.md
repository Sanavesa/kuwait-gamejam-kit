...
**1. Quick pre-flight.** Make sure the game actually runs (no console errors), and run `/save` first so the submitted version is saved. If we haven't tried it recently, remind me to **open `index.html` and play a full round myself first** (and hand it to someone else to play) so I catch anything broken before judges do.
