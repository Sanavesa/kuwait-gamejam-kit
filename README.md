...
A beginner-friendly game-dev toolkit for Claude Code. It turns the whole jam (idea, build, polish, ship) into 7 one-word commands, and primes every session into "game-jam mode" — where Claude also handles art and sound for you as the game grows. No engine, no build tools, no clutter.

> **You only need 5 to start:** brainstorm an idea (`/brainstorm`), scaffold it (`/new-game`), build it in a loop with `/fix` and `/save`, add polish (`/polish`), and ship it (`/submit`). The rest are there when you reach for them.

## Contents
- [Install](#install): get set up in two commands
- [How to use it](#how-to-use-it): your first game, start to finish
- [Command reference](#command-reference): all 7 commands at a glance
...
```
That's it. All 7 commands are live in every project. Next: [How to use it](#how-to-use-it).
...
6. **Playtest it with real people.** Hand it to a friend and say nothing. Watch where they get confused; that's your fix list. (You're the first playtester too: do the first 30 seconds make sense with no instructions?) Fix what you saw with `/fix` and `/polish`.
