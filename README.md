...
A beginner-friendly game-dev toolkit for Claude Code. It turns the whole jam (idea, build, polish, ship) into 6 one-word commands, and primes every session into "game-jam mode" where Claude also handles art and sound for you as the game grows. No engine, no build tools, no clutter.

> **You only need 4 to start:** start a game (`/new-game`), where Claude brainstorms the idea with you and then builds it, then loop with `/fix` and `/save`, add polish (`/polish`), and ship it (`/submit`). The rest are there when you reach for them.

## Contents
- [Install](#install): get set up in two commands
- [How to use it](#how-to-use-it): your first game, start to finish
- [Command reference](#command-reference): all 6 commands at a glance
...
```
That's it. All 6 commands are live in every project. Next: [How to use it](#how-to-use-it).
...
4. **Make it feel and look good** (this is where the points are): run `/polish`, then pick the effects you want from the menu it shows (shake, particles, pops, and more). For **sound and art, just ask** (*"add a coin sound"*, *"give the player a ship sprite"*), and Claude generates audio (Web Audio) and art (code-drawn, cohesive palette) right in the game, or wires in any file you drop in the folder; this is on by default every session, no command needed. Run one thing at a time, play, then `/save`. Need a title, pause, or game-over screen? Just ask, and Claude builds the whole game shell with a saved high score. Crisp scaling to any screen is already built in.
5. **Playtest it with real people.** Hand it to a friend and say nothing. Watch where they get confused; that's your fix list. (You're the first playtester too: do the first 30 seconds make sense with no instructions?) Fix what you saw with `/fix` and `/polish`.
...
| Command | Does |
|---|---|
| `/polish [moment]` | Shows a menu of game-feel effects (shake, particles, pops, hit-pause, flashes) and adds the ones you pick. |

> **Art & sound need no command.** Just ask in plain words (*"add a coin sound"*, *"give the player a ship sprite"*) and Claude generates audio (Web Audio) and art (code-drawn, cohesive palette) right in the game, or wires in any file you drop in the folder. This is on by default in every session.
