...
A beginner-friendly game-dev toolkit for Claude Code. It turns the whole jam (idea, build, polish, ship) into 4 one-word commands, and primes every session into "game-jam mode" where Claude also handles running, art, sound, polish, and bug-fixing for you as the game grows. No engine, no build tools, no clutter.

> **You only need 3 to start:** start a game (`/new-game`), where Claude brainstorms the idea with you and then builds it, then loop by just talking (ask for changes, ask to run it, paste any error to fix it) and checkpointing with `/save`, and ship it (`/submit`). Running, polish, art, sound, and bug-fixing just happen as you build, no command needed. The rest are there when you reach for them.
...
3. **The build loop (no command needed).** Adding features is just talking. Ask in plain words: *"add a double jump"*, *"make the pipes spawn slower"*, *"add a score +1 per pipe."* Claude keeps each change small, won't break what already works, and reminds you to play and checkpoint. Play after every change, just ask Claude to run it (if a change doesn't show up, hard-refresh the browser with **Ctrl/Cmd+Shift+R**, you're seeing a cached old version). Like it? `/save score works`. Broke it? Just say what's wrong (*"the bird falls through the floor"*) or paste the console error and Claude fixes it, or `/undo`.
...
| Command | Does |
|---|---|
| `/new-game <idea/theme>` | Brainstorms the idea with you (core mechanic, feeling, theme fit), then scaffolds a playable single-file `index.html`, writes a `CLAUDE.md` game brief, sets up git, runs it for you, and saves the first checkpoint. |
...
> **Running the game needs no command.** Just ask Claude to run it (or it offers after a change). It starts a local preview server and hands you a `http://localhost` link, so multi-file games and audio work the same way itch.io will serve them, no double-clicking the file, no CORS errors. After a change doesn't show up, hard-refresh with **Ctrl/Cmd+Shift+R**.
...
| **The game won't open / "CORS" error** | Ask Claude to run it, it serves the game properly over `http://localhost` instead of you opening the file directly. |
